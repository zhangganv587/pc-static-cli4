 ...
 * @LastEditors: 张淦
 * @Description: 商户系统 - 渠道账号管理 - 收款账号管理 - 微信收款账号管理
 * @FilePath: /gpay-ops-static/src/pages/merchant/channelManage/wechatCollection.vue
 -->
<template>
  <div class="wx-collection">
    ...
    <v-add-btn @click="handleInfo({}, 'wechatAccount')">
      添加微信收款账号
    </v-add-btn>
    <v-table
      v-loading="loading"
    ...
    />
    <wechatAccountDialog :row-data="row" />
  </div>
</template>
...
import { AVAILABLE_STATE, DICT_STATUS, AVAILABLE_STATE_ARR, WX_PAY_WAY_CODE_ARR } from '@/utils/constants.js';
import wechatAccountDialog from './component/wechatAccountDialog';
          ...
          render: (h, value, row) => {
            return (
              <div>
                ...
                <el-button onClick={() => this.handleInfo(row, 'wechatAccount')} type="text">详情</el-button>
              </div>
            );
          }
        },
