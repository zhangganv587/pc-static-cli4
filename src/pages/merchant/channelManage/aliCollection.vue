<!--
 * @Author: 张淦
 * @Date: 2020-04-29 14:27:03
 * @LastEditors: zhanggan
 * @Description: 商户系统 - 渠道账号管理 - 收款账号管理 - 支付宝收款账号管理
 * @FilePath: /gpay-ops-static/src/pages/merchant/channelManage/aliCollection.vue
 -->
<template>
  <div class="ali-collection">
    ...
    <v-add-btn @click="handleInfo({}, 'aliAccount')">
      添加支付宝收款账号
    </v-add-btn>
    <v-table
      v-loading="loading"
    ...
    />
    <aliAccountDialog :row-data="row" />
    <aliOpenPlatform :row-data="row" />
    <aliMapiDialog :row-data="row" />
    <aliWapDialog :row-data="row" />
  </div>
</template>
...
import { AVAILABLE_STATE, DICT_STATUS, AVAILABLE_STATE_ARR, ALI_PRODUCT_TYPE_ARR } from '@/utils/constants.js';
import aliAccountDialog from './component/aliAccountDialog';
import aliOpenPlatform from './component/aliOpenPlatform';
import aliMapiDialog from './component/aliMapiDialog';
import aliWapDialog from './component/aliWapDialog';
...
export default {
  ...
  data() {
    ...
    this.queryUrl = '/pay/account/query.json';
    ...
    this.productTypeArr = ALI_PRODUCT_TYPE_ARR;
    return {
      creatTime: null,
      modifyTime: null,
      // 搜索form
          ...
          render: (h, value, row) => {
            return (
              <div>
                ...
                <el-button onClick={() => this.handleInfo(row, 'aliAccount')} type="text">详情</el-button>
                <el-button onClick={() => this.handleInfo(row, 'aliOpenPlatform')} type="text">开放平台秘钥</el-button>
                <el-button onClick={() => this.handleInfo(row, 'aliMapi')} type="text">mapi网关产品秘钥</el-button>
                <el-button onClick={() => this.handleInfo(row, 'aliWap')} type="text">老版本wap支付秘钥</el-button>
              </div>
            );
          }
        },
